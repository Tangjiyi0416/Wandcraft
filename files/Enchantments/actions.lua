...
table.insert( actions,{
	...
	action 	= function()
		local abi_com = GetCurrentWandAbilityComponent()
		if abi_com~=nil then  --credit kermit tears for solving the first time loading problem
			local fire_rate_wait = ComponentObjectGetValue(abi_com, "gun_config", "fire_rate_wait")
			ComponentObjectSetValue(abi_com , "gun_config", "fire_rate_wait", tostring((tonumber(fire_rate_wait)-10)))
